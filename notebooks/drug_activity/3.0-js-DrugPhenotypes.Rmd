...
author: "Jan Sauer and Niklas Rindtorff"
date: "`r Sys.Date()`"
output:
  html_document:
      code_folding: hide
...
library(knitr)
library(igraph)
#library(qgraph) # Running the vignette with fruchterman-reingold clustering to avoid gfortran hickups in R.
library(Matrix)
library(tidyverse)
library(Rtsne)
...
# Load data
utils::data("drug_effects", package = "SCOPEAnalysis")
...
save_images = TRUE
img_out_dir = here::here("reports/figures/drug_activity")
...
```{r auroc_hist}
ggplot(data = drug_effect_metadata_filtered, mapping = aes(x = AUC_Mean)) + 
...
```{r dist_vs_auroc}
ggplot(data = drug_effect_metadata_filtered, 
       mapping = aes(x = AUC_Mean, y = Distance)) + 
  geom_point(size = 0.5) + 
  theme_vignette() + labs(
    caption = paste("Spearman rho =", round(
      cor(drug_effect_metadata_filtered$AUC_Mean, 
          drug_effect_metadata_filtered$Distance, method = "spearman"), 2))) + 
...
profiles_active_filtered = drug_effect_metadata_filtered[drug_effect_metadata_filtered$AUC_Mean >= auc_thresh, ]
metadata_active_filtered = drug_effect_metadata_filtered[drug_effect_metadata_filtered$AUC_Mean >= auc_thresh, ]

```

The number of active drugs varies per line.
```{r active_drugs_per_line}
ggplot_df = as.data.frame(table(metadata_active_filtered$Line))
...
ggplot_df %>% write_csv(here::here("reports/tables/source_data_Barplot_NumActiveDrugs_3b.csv"))
...
activity_table = table(metadata_active_filtered$Drug, metadata_active_filtered$Line)
...
ggplot(mapping = aes(y = Viability, x = AUC_Mean)) + 
  geom_point(data = drug_effect_metadata_filtered, color = "lightgrey", size = 0.5) + 
  geom_point(data = drug_effect_metadata_filtered[
    drug_effect_metadata_filtered$AUC_Mean >= auc_thresh, ], 
             size = 0.5) + 
...
I look at the angles between vectors. The classification generates a profile for all drugs, including inactive ones. However, because they cannot be properly separated from DMSO, the profiles of these drugs will be largely random, ergo I remove them as noise.
...
heatmap_pathways = oddsratio_pathway[, c("Line", "Label", "OddsRatio")] %>% filter(!Line %in% c("D021T01", "D054T01", "D055T01", "D020T02"))
...
heatmap_targets = oddsratio_target[, c("Line", "Label", "OddsRatio")] %>% filter(!Line %in% c("D021T01", "D054T01", "D055T01", "D020T02"))
...
```{r phenoprints_pathways, eval = FALSE}
...
```{r phenoprints_targets, eval = FALSE}
...
if(save_images) pheatmap(
    ...
    cluster_rows = hc, cluster_cols = hc, 
    main = sprintf("Angles Between Profile Vectors for all lines"), 
    border_color = NA, cellwidth = 1.5, cellheight = 15, height = 150, 
    filename = file.path(img_out_dir, "EffectVectorHeatmap_allDrugs_large.pdf"))
```

```{r}
active_drugs = rownames(d)

drug_df <- readRDS(here::here("data/processed/morphology/pca_absolute_all_drugs_aggregate.Rds")) %>% 
  ...
  ungroup() %>% 
  gather(pca, value, -line, -drug) %>%
  mutate(feature = paste0(pca, "_", line)) %>% 
  dplyr::select(-line, -pca)

active_drug_df <- drug_df %>% 
  filter(drug %in% active_drugs)

drug_df_cor <- active_drug_df %>%
  spread(drug, value) %>%
  column_to_rownames("feature") %>% 
  cor()

pheatmap(
    mat = drug_df_cor, annotation_col = annotation %>% dplyr::select(-Pathway), 
    annotation_colors = anno_colorScale, 
    show_rownames = FALSE, show_colnames = FALSE, 
    #cluster_rows = hc, cluster_cols = hc, 
    main = sprintf("Aggregated drug-induced PCA Profile Vectors for all lines"), 
    border_color = NA)

if(save_images) pheatmap(
    drug_df_cor, annotation_col = annotation %>% dplyr::select(-Pathway), 
    #color = hm_colorscale, 
    annotation_colors = anno_colorScale, 
    show_rownames = FALSE, show_colnames = FALSE, 
    #cluster_rows = hc, cluster_cols = hc, 
    main = sprintf("Aggregated drug-induced PCA Profile Vectors for all lines"), 
    border_color = NA, cellwidth = 1.5, cellheight = 1.5, height = 15, 
    filename = file.path(img_out_dir, "aggregate_pca_heatmap_allDrugs.pdf"))
...
if(save_images) pheatmap(
    ...
    main = sprintf("Aggregated drug-induced PCA Profile Vectors for all lines"), 
    border_color = NA, cellwidth = 1.5, cellheight = 15, height = 150, 
    filename = file.path(img_out_dir, "aggregate_pca_heatmap_allDrugs_large.pdf"))
```
...
```{r graph_of_panline_drug_similarities, eval = FALSE}
# Set up adjacency matrix
adjmatrix = d
adjmatrix[adjmatrix > 45] = 0
...
```{r ccp_conc_heatmaps, eval = FALSE}
