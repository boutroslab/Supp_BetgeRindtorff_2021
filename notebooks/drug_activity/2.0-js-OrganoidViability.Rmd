...
author: "Jan Sauer, edited by Niklas Rindtorff"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
...
save_images = TRUE
img_output_dir = here::here("reports/figures/drug_viability")
if(save_images) if(!dir.exists(img_output_dir)) dir.create(img_output_dir)
```

# QC

I remove a set of organoid lines from the data

```{r}
auc_ctg <- auc_ctg %>% filter(!Line %in% c("D021T01", "D054T01", "D055T01"))
auc_img_full <- auc_img_full %>% filter(!Line %in% c("D021T01", "D054T01", "D055T01"))
auc_img_twoChannel <- auc_img_twoChannel %>% filter(!Line %in% c("D021T01", "D054T01", "D055T01"))
mortality <- mortality %>% filter(!Line %in% c("D021T01", "D054T01", "D055T01"))
roc_aucs <- roc_aucs %>% filter(!clf_line %in% c("D021T01", "D054T01", "D055T01")) %>% filter(!data_line %in% c("D021T01", "D054T01", "D055T01"))
roc_data <- roc_data %>% filter(!clf_line %in% c("D021T01", "D054T01", "D055T01")) %>% filter(!data_line %in% c("D021T01", "D054T01", "D055T01"))
```
...
drug_viability = mortality %>% 
  ...
  filter(Line != "D054T01") %>%
  dplyr::select(Product.Name, Line, Percent.Live)
...
```{r, eval=T}
index_acc_matrix = !(colnames(acc_matrix) %in% c("D021T01", "D054T01", "D055T01"))

hm_colorScale = colorRampPalette(
  c("#f7f7f7", "#003399"))(150)
pheatmap(
  acc_matrix[index_acc_matrix, index_acc_matrix], cluster_rows = FALSE, cluster_cols = FALSE, 
...
```{r, eval=TRUE}
# rerunning just in case
index_acc_matrix = !(colnames(acc_matrix) %in% c("D021T01", "D054T01", "D055T01"))

acc_full = diag(acc_matrix[index_acc_matrix, index_acc_matrix])
acc_Actin = diag(acc_matrix_reduced$Actin[index_acc_matrix, index_acc_matrix])
acc_FITC = diag(acc_matrix_reduced$FITC[index_acc_matrix, index_acc_matrix])
acc_DAPI = diag(acc_matrix_reduced$DAPI[index_acc_matrix, index_acc_matrix])
acc_ActinDAPI = diag(acc_matrix_reduced$Actin_DAPI[index_acc_matrix, index_acc_matrix])
...
# focus on D030T01
ggplot_df_focus <- ggplot_df %>% filter(Line == "D030T01") %>%
  filter(Concentration != 1)

for(drug in drugs) {
  gp = ggplot(data = ggplot_df_focus[ggplot_df_focus$Drug == drug, ]) +
    geom_ribbon(aes(x = ConcentrationInt, ymin = Min, ymax = Max,
                    color = Type, fill = Type), alpha = 0.25) +
    geom_line(aes(x = ConcentrationInt, y = Mean, color = Type), size = 2) +
    scale_x_continuous(labels = conc_inv_transform) +
  ...
  gp + ggsave(
  filename = file.path(
    img_output_dir, "Outliers_D030T01_MTX_doseresponse.pdf"), 
  width = 5, height = 5, useDingbats = FALSE)
}
