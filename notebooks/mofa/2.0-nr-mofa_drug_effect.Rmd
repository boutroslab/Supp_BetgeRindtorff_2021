...
moa_factor_shift %>% 
  mutate(fdr_bool = if_else(fdr < 0.05, TRUE, FALSE)) %>%
  filter(factor == "factor1") %>%
  write_csv(here::here("reports/tables/source_data_f1_moa_shift_6f.csv"))
...
moa_factor_shift %>% 
  mutate(fdr_bool = if_else(fdr < 0.05, TRUE, FALSE)) %>%
  filter(factor == "factor2") %>%
  write_csv(here::here("reports/tables/source_data_f2_moa_shift_5f.csv"))
...
z_projected_conc_anno_tidy %>% 
  ...
  dplyr::select(concentration, V2) %>%
  write_csv(here::here("reports/tables/source_data_f2_mek_shift_5g.csv"))
```
