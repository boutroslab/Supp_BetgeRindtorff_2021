...
# organoid mutation
mofa_genetics <- read_delim(here::here("data/processed/seq/Table-S2_Mutations_PDOs_RevisionV4.csv"), delim = ";") %>% 
    janitor::clean_names() %>% 
...
df %>% write_csv(here::here("reports/tables/source_data_var_explained_feature_4b.csv"))
```
...
model@cache$variance_explained$r2_per_factor$single_group %>% as.data.frame() %>% 
    rownames_to_column("factor") %>% write_csv(here::here("reports/tables/source_data_var_explained_4a.csv"))
...
df %>% write_csv(here::here("reports/tables/source_data_f1_size_6a.csv"))
...
drug_activity_joined %>% 
  filter(drug %in% c("OSI-420", "Trametinib (GSK1120212)", "BMS-536924")) %>% 
  write_csv(here::here("reports/tables/source_data_f1_drug_pick_6e.csv"))
```
...
drug_activity_joined %>% 
  filter(drug %in% c("PRI-724", "Trametinib (GSK1120212)", "Ulixertinib (BVD-523, VRT752271)")) %>% 
  write_csv(here::here("reports/tables/source_data_f2_drug_pick_5e.csv"))
```
